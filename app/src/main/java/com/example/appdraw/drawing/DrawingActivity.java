...
public class DrawingActivity extends AppCompatActivity {
            ...
            com.google.firebase.firestore.FirebaseFirestore.getInstance()
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        com.example.appdraw.model.Artwork artwork = documentSnapshot.toObject(com.example.appdraw.model.Artwork.class);
                        if (artwork != null) {
                            TextView txtProjectName = findViewById(R.id.txtProjectName);
                            if (txtProjectName != null && artwork.getTitle() != null && !artwork.getTitle().isEmpty()) {
                            ...
                            if (artwork.getImageUrl() != null && artwork.getImageUrl().startsWith("data:image")) {
                                ...
                                if (bmp != null) {
                                    drawingView.post(() -> {
                                        com.example.appdraw.ZoomDrawingView.Layer activeLayer = drawingView.getActiveLayer();
                                        if (activeLayer != null) {
                                            activeLayer.strokes.add(new com.example.appdraw.ZoomDrawingView.Stroke(bmp));
            ...
            public void run() {
                ...
                for (int i = layers.size() - 1; i >= 0; i--) {
                    ...
                    if (layerIndex == activeIndex) {
                        row.setBackgroundColor(Color.parseColor("#E8F0FE")); // Màu xanh nhạt để làm nổi bật lớp đang chọn
                    } else {
                    ...
                    nameView.setTextColor(layerIndex == activeIndex ? Color.parseColor("#1A73E8") : Color.parseColor("#3C4043"));
                    if (layerIndex == activeIndex) nameView.setTypeface(null, android.graphics.Typeface.BOLD);
    ...
    private void showPenSettingsDialog() {
        ...
        Runnable updateHardnessSelectionUI = () -> {
            for (int i = 0; i < hardnessGroup.getChildCount(); i++) {
                ...
                if (child.getTag() != null && child.getTag().equals(selectedHardness[0].name())) {
                    ...
                    child.setSelected(false);
                    if (child instanceof TextView) ((TextView)child).setTextColor(Color.parseColor("#757575"));
                }
            }
        };
        ...
        if (btnCancelPen != null) btnCancelPen.setOnClickListener(v -> dialog.dismiss());
        if (btnSavePen != null) {
            btnSavePen.setOnClickListener(v -> {
                ...
                if (toolPen != null) {
                    ...
                    switch (selectedType[0]) {
                        case PENCIL: resId = R.drawable.ic_brush_pencil; name = "Bút chì"; break;
                        case WATERCOLOR: resId = R.drawable.ic_brush_watercolor; name = "Bút lông"; break;
                        case CRAYON: resId = R.drawable.ic_brush_crayon; name = "Bút sáp"; break;
                        case INK: resId = R.drawable.ic_brush_ink; name = "Bút mực"; break;
                        case MARKER: resId = R.drawable.ic_brush_marker; name = "Đánh dấu"; break;
                        case AIRBRUSH: resId = R.drawable.ic_brush_airbrush; name = "Xịt sơn"; break;
                        case CALLIGRAPHY: resId = R.drawable.ic_brush_calligraphy; name = "Thư pháp"; break;
                        case OIL: resId = R.drawable.ic_brush_oil; name = "Bút dầu"; break;
                    }
    ...
    private void showColorPickerDialog() {
        ...
        if (!recentStr.isEmpty()) {
            for (String s : recentStr.split(",")) {
                try { recentColors.add(Integer.parseInt(s)); } catch (Exception ignored) {}
            }
        }
        ...
        if (btnSave != null) {
            btnSave.setOnClickListener(v -> {
                ...
                if (recentColors.contains(selected)) recentColors.remove((Integer) selected);
                recentColors.add(0, selected);
                if (recentColors.size() > 7) recentColors.remove(recentColors.size() - 1);
                
                StringBuilder sb = new StringBuilder();
                for (int c : recentColors) sb.append(c).append(",");
    ...
    private void selectTool(LinearLayout selectedTool) {
        if (toolPen != null) toolPen.setBackground(null);
        if (toolEraser != null) toolEraser.setBackground(null);
        if (toolFill != null) toolFill.setBackground(null);
        if (toolClear != null) toolClear.setBackground(null);
    ...
    private void resetToolColor(LinearLayout tool, String colorStr) {
        ...
        for (int i = 0; i < tool.getChildCount(); i++) {
            ...
            if (child instanceof ImageView) {
                ...
                tv.setTypeface(null, colorStr.equals("#1A73E8") ? android.graphics.Typeface.BOLD : android.graphics.Typeface.NORMAL);
            }
        }
    }
    ...
    private void saveImageToGallery() {
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                ...
                contentValues.put(android.provider.MediaStore.MediaColumns.DISPLAY_NAME, "AppDraw_" + System.currentTimeMillis() + ".png");
                contentValues.put(android.provider.MediaStore.MediaColumns.MIME_TYPE, "image/png");
                contentValues.put(android.provider.MediaStore.MediaColumns.RELATIVE_PATH, android.os.Environment.DIRECTORY_PICTURES + "/AppDraw");
                ...
                String imagesDir = android.os.Environment.getExternalStoragePublicDirectory(android.os.Environment.DIRECTORY_PICTURES).toString();
            ...
            if (fos != null) {
                fos.flush();
                fos.close();
                android.widget.Toast.makeText(this, "Đã lưu ảnh vào Thư viện", android.widget.Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            android.widget.Toast.makeText(this, "Lỗi khi lưu ảnh: " + e.getMessage(), android.widget.Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
    ...
    private void showSaveArtworkDialog() {
        ...
        android.widget.ArrayAdapter<String> spinnerAdapter = new android.widget.ArrayAdapter<>(this, android.R.layout.simple_spinner_item, spinnerAdapterList);
                ...
                com.google.firebase.firestore.FirebaseFirestore.getInstance().collection("Projects")
                        ...
                        .addOnSuccessListener(queryDocumentSnapshots -> {
                            ...
                            for (com.google.firebase.firestore.QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                                try {
                                    com.example.appdraw.model.Project p = doc.toObject(com.example.appdraw.model.Project.class);
            ...
            if (passedProjectId != null) {
                ...
                if (selectedPos == 0) {
                    ...
                    if (newProjName.isEmpty()) {
                        android.widget.Toast.makeText(this, "Vui lòng nhập tên dự án mới", android.widget.Toast.LENGTH_SHORT).show();
                        return;
                    }
                    dialog.dismiss();
                    ...
                    android.widget.Toast.makeText(this, "Vui lòng đợi dữ liệu tải xong", android.widget.Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void createNewProjectAndSave(String projName, String artworkName, String selectedStatus) {
        ...
        com.example.appdraw.model.Project newProj = new com.example.appdraw.model.Project(docId, uid, projName, "", "", "", 0, System.currentTimeMillis());
        ...
        db.collection("Projects").document(docId).set(newProj)
                ...
                .addOnFailureListener(e -> {
                    android.widget.Toast.makeText(this, "Lỗi tạo dự án: " + e.getMessage(), android.widget.Toast.LENGTH_SHORT).show();
                });
    }
    ...
    private void doSaveArtworkProcess(String projectId, String artworkId, String artworkName, String selectedStatus) {
        ...
        final String targetArtworkId = (artworkId != null && !artworkId.isEmpty()) ? artworkId : db.collection("Artworks").document().getId();
        ...
        if (artworkId != null && !artworkId.isEmpty()) {
            db.collection("Artworks").document(artworkId)
                ...
                .addOnSuccessListener(aVoid -> {
                     android.widget.Toast.makeText(this, "Đã lưu tác phẩm thành công!", android.widget.Toast.LENGTH_SHORT).show();
                     finish();
                }).addOnFailureListener(e -> {
                     android.widget.Toast.makeText(this, "Lỗi lưu DB: " + e.getMessage(), android.widget.Toast.LENGTH_LONG).show();
                });
        } else {
            com.example.appdraw.model.Artwork newArtwork = new com.example.appdraw.model.Artwork(targetArtworkId, uid, projectId, artworkName, base64Image, selectedStatus, System.currentTimeMillis());
